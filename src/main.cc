...
int main(int argc ,char *argv[]){
    ...
    if(argc != 3){
        ...
        cout << "[Offline] USAGE is : " << program << " Doffset_low Doffset_high/n";
        return -1;
    } else if(argc == 3){
        int Doffset_low = 0;
        int Doffset_high = 0;
        ...
        for(int d=Doffset_low; d<=Doffset_high; d++){
            string ROOTName = "Angle-dependency_" + intTostring(d) + ".root";
            ...
            for(unsigned int a=0; a<60; a+=5){
                ...
                while(n<Nmuons){
                    muonPos = getRandomMuonPosition(generator,__scint_H+__scint_W*sin(alpha)/2.);
                    direction = getRandomDirection(generator);

                    bool in_trigger = isInScint(muonPos,direction,alpha,d,"TOP") && isInScint(muonPos,direction,alpha,d,"BOTTOM");

                    if (in_trigger){
                        GroundPos = getHitPosition(muonPos,direction,alpha,d,"GROUND");
                        GroundProfil->Fill(GroundPos.x,GroundPos.y);

                        ScreenPos = getHitPosition(muonPos,direction,alpha,d,"SCREEN");
